...
setwd("C:/Users/noe.nava/OneDrive - USDA/Drive/courses/vehtari_bayesian_data_analysis/BDA_course_Aalto/navaAssignments/assignment 4/bda_r_demos")
...
r <- c(1.1, 1.3, -0.1, -0.7, 0.2, -0.4, 0.06, -1.7, 1.7, 0.3, 0.7, 1.6, -2.06, -0.74, 0.2, 0.5)
...
# Note: We can draw from the probability density proportional to g. It is not required
# that g(theta) integrate to 1, but g(theta) must have a finite integral
g_mean <- 0
g_sd   <- 1.1
g      <- dnorm(x, g_mean, g_sd)

# M is computed by discrete approximation
# The importance ratio p(theta|y)/g(theta) must have a known bound; that is, there
# must be some known constant M for which p(theta|y)/g(theta)=<M for all theta.
M <- max(q/g)
g <- g*M # to make sure p(theta|y)/g(theta)=<M
...
### Sampling:
# 1. Sample theta at random from the probability density proportional to g(theta)
# 2. With probability p(theta|y)/Mg(theta), accept theta as a draw from p. If
# draw is rejected, then return to step one
sims <- 100
prob <- q/g
df <- data.frame()
for(s in 1:sims) {
  a <- sample(g, 1)
  a <- which.min(abs(x - a)) # find the closest grid point
  a <- ifelse(g[a] > prob[a], a, NA)
  df <- rbind.data.frame(df, g[a])
}
df
